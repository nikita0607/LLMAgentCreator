...
export default function AgentEditorPage() {
  ...
  // Ищем полную ноду по id в массиве nodes
  const fullNode = nodes.find((n) => n.id === node.id);

  if (fullNode) {
    // Копируем все поля data + id
    setEditNode({ ...fullNode.data, id: fullNode.id });
  } else {
    // fallback для новых узлов
  ...
  try {
    ...
    if (data.logic) {
      // создаём ReactFlow-ноды
      const agentNodes: Node[] = data.logic.nodes.map((n: any, i: number) => {
        let paramsArray: any[] = [];
        if (n.type === "webhook" && n.params) {
          // преобразуем объект params -> массив {name, description/value}
          console.log("PARAMS:", n.params)
          paramsArray = n.params.map((el) => {
                el.id = crypto.randomUUID();
                const param: NodeParam = el;
                return param;
              });
          console.log("ARR", paramsArray);
        }
      ...
      // строим edges из полей next / on_success / on_failure
      const agentEdges: Edge[] = [];
      data.logic.nodes.forEach((n: any) => {
          console.log("KLOADIN", n)
...
const addNode = () => {
  ...
  openModal(newNode); // сразу открываем модалку
};
...
const saveNode = () => {
  ...
  try {
    // словарь: у какой ноды какие исходящие связи
    ...
    // находим стартовую ноду (у которой нет входящих рёбер)
    ...
    const logicNodes = nodes.map((n) => {
      ...
      if (data.type === "message") {
        const out = outgoing[n.id]?.[0]; // у message только одно ребро
      ...
      if (data.type === "webhook") {
        ...
        if (data.missing_param_message) base.missing_param_message = data.missing_param_message;

        // params: превращаем массив в объект
        if (data.params && data.params.length > 0) {
          base.params = data.params;
        }

        // ребра webhook: success / failure
    ...
    console.log(payload);
    await apiFetch(`/agents/${agentId}`, { method: "PUT", body: JSON.stringify(payload) });
    ...
    <ReactFlowProvider>
      <div className="h-screen flex flex-col bg-gray-100 p-4">
        <div className="flex justify-between items-center mb-2">
          <h1 className="text-2xl font-bold text-gray-900">Редактор агента: {agentName}</h1>
          <div className="flex gap-2">
            <button className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600" onClick={addNode}>Добавить узел</button>
            <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600" onClick={saveToServer}>Сохранить на сервер</button>
          </div>
        </div>
          ...
          fitView
           onNodeDoubleClick={(event, node) => {
               event.stopPropagation();
                openModal(node); // передаем node.data в модалку
  }}
          nodeTypes={nodeTypes}
        >
          <MiniMap />
        ...
        <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} className="fixed z-10 inset-0 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            <Dialog.Panel className="bg-white rounded shadow-lg p-6 w-full max-w-md">
              <Dialog.Title className="text-xl font-bold mb-4">Редактировать узел</Dialog.Title>
              {editNode && (
                <>
                  ...
                  <select
                    value={editNode.type}
                    onChange={(e) => setEditNode({ ...editNode, type: e.target.value as "message" | "webhook" })}
                  ...
                  {editNode.type === "webhook" && (
                    <>
                      <div className="mb-4">
                        <h3 className="font-semibold mb-2 text-gray-800">Параметры Webhook</h3>
                        {(editNode.params || []).map((param, idx) => (
                          <div key={param.id} className="flex gap-2 mb-2">
                            <input type="text" placeholder="Название" className="flex-1 p-2 border rounded text-gray-900"
                              value={param.name}
                              onChange={(e) => {
                            ...
                            />
                            <input type="text" placeholder="Описание" className="flex-2 p-2 border rounded text-gray-900"
                            ...
                            />
                            <input type="text" placeholder="Фиксированное значение (опционально)" className="flex-2 p-2 border rounded text-gray-900"
                              value={param.value || ""}
                              onChange={(e) => {
                                const newParams = [...(editNode.params || [])];
                            ...
                            />
                            <button type="button" className="bg-red-500 text-white px-2 rounded hover:bg-red-600"
                              onClick={() => {
                                const newParams = [...(editNode.params || [])];
                        ...
                        <button type="button" className="bg-green-500 text-white px-4 py-1 rounded hover:bg-green-600"
                          onClick={() => {
                            const newParams = [...(editNode.params || []), {id: crypto.randomUUID(), name: "", description: "", value: "" }];
                            setEditNode({ ...editNode, params: newParams });
                          }}
                        >Добавить параметр</button>

                        <label className="block mb-1 text-gray-800 mt-2">Action</label>
                        <input type="text" className="w-full p-2 border rounded mb-2 text-gray-900"
                          value={editNode.action || ""}
                          onChange={(e) => setEditNode({ ...editNode, action: e.target.value })}
                        />
                        <label className="block mb-1 text-gray-800">URL</label>
                        <input type="text" className="w-full p-2 border rounded mb-2 text-gray-900"
                          value={editNode.url || ""}
                          onChange={(e) => setEditNode({ ...editNode, url: e.target.value })}
                        />
                        <label className="block mb-1 text-gray-800">Method</label>
                        <select className="w-full p-2 border rounded mb-2 text-gray-900"
                        ...
                        <label className="block mb-1 text-gray-800">Сообщение при нехватке параметров</label>
                        <input type="text" className="w-full p-2 border rounded mb-2 text-gray-900"
                          value={editNode.missing_param_message || ""}
                          onChange={(e) => setEditNode({ ...editNode, missing_param_message: e.target.value })}
                        />
                      </div>
                    </>
                  )}
                  ...
                  <div className="flex justify-end gap-2 mt-2">
                    <button className="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400" onClick={() => setIsModalOpen(false)}>Отмена</button>
                    <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600" onClick={saveNode}>Сохранить</button>
                  </div>
                </>
              )}
            </Dialog.Panel>
